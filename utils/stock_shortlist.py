...
def run_fundamental_shortlist(db):
    ...
    Returns a summary including a golden/silver breakdown and aggregate
    exclusion-reason counts (e.g. how many were excluded on PE range) for
    the route to report. failed_criteria_counts only reflects companies
    that were actually excluded (tier is None) -- a silver company's
    PE/OPM "failure" isn't counted there, since it didn't cause exclusion."""
    db.execute(
    ...
    for row in candidates:
        ...
        if tier:
            ...
            db.execute(
                ...
                (symbol, exchange, row.get('company_name'), SHORTLIST_SOURCE, tier,
