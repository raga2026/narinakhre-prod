...
def test_subscription_welcome_email_without_suggestions_is_the_plain_welcome():
    ...
    assert 'no list to show yet' in kwargs['textbody'].lower()
...
def test_subscription_welcome_email_with_suggestions_includes_todays_pick():
    ...
    kwargs = mock_send.call_args.kwargs
    assert 'highly recommended' in kwargs['subject'].lower()
...
def test_subscription_welcome_email_defaults_to_pro_plan_pricing():
    with patch('stoqbell.utils.suggestion_email.send_zeptomail_stocks_email', return_value=(True, 'ok')) as mock_send:
        send_subscription_welcome_email('new@example.com', 'New Sub', '17 Sep 2026')

    kwargs = mock_send.call_args.kwargs
    assert 'Rs 299 + GST (Rs 352.82)/month' in kwargs['textbody']
    assert 'StoqBell Pro subscription is active' in kwargs['textbody']
...
class FakeLargeCapBonusEmailDB:
    ...
    filter on stocks_plan='pro', not just 'every active viewer'."""
    ...
    def execute(self, sql, params=None):
        ...
        if "stocks_plan='pro'" in normalized and normalized.startswith("SELECT id, username AS email, name FROM stocks_admin_users WHERE role='viewer'"):
            self.last_recipient_sql = normalized
            if 'AND id IN (' in normalized:
                wanted = set(params)
                return FakeCursor([r for r in self.recipient_rows if r.get('id') in wanted])
...
def test_large_cap_bonus_email_sends_only_to_pro_recipients():
