...
class FakeAuthDB:
    ...
    def execute(self, sql, params=None):
        ...
        if normalized.startswith(
            ...
            email, name, google_sub, referred_by_id, stocks_plan = params
            self.rows.append({
                ...
                'google_sub': google_sub, 'referred_by_id': referred_by_id, 'stocks_plan': stocks_plan,
            })
            self._next_id += 1
            return FakeCursor([])
        ...
        if normalized.startswith(
            ...
            self.rows.append({
                'id': self._next_id, 'username': email, 'name': name, 'role': 'viewer',
                'is_active': 0, 'can_view_watchlist': 0, 'must_change_password': 0, 'is_pro': 0,
...
def test_authenticate_correct_credentials_returns_row_and_no_reason():
    db = FakeAuthDB(rows=[{
        ...
        'trial_ends_at': None, 'stocks_plan': 'standard',
    }])
...
def test_authenticate_expired_trial_returns_trial_expired_not_invalid():
    ...
    db = FakeAuthDB(rows=[{
        ...
        'trial_ends_at': past, 'stocks_plan': 'standard',
    }])
    row, reason = authenticate_stocks_admin(db, 'a@example.com', 'secret123')
    assert row is None
...
def test_google_signup_grants_trial_for_standard():
    db = FakeAuthDB()
    row = create_pending_google_subscriber(db, 'a@example.com', 'A', 'sub_123', stocks_plan='standard')
    assert row['subscription_status'] == 'trialing'
    assert row['is_active'] == 1
    assert row['trial_ends_at'] is not None


def test_google_signup_keeps_starters_pending():
    db = FakeAuthDB()
    row = create_pending_google_subscriber(db, 'a@example.com', 'A', 'sub_123', stocks_plan='starters')
