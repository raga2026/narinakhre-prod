...
# recaptcha_shared/razorpay_shared read env vars at import time -- imported
# here (after load_env_file above), not at the top of the file with this
# module's other imports, so .env is already loaded by the time they run.
from recaptcha_shared import verify_recaptcha, RECAPTCHA_SITE_KEY, RECAPTCHA_SECRET_KEY

# Nari Nakhre Stocks lives on its own Blueprint (stoqbell/routes.py) --
# imported here for the same reason, since stoqbell/routes.py also reads
# env vars and builds a Razorpay client at import time.
from stoqbell.routes import stocks_bp, register_legacy_stocks_routes, init_stocks_tables, STOCKS_DOMAIN
app.register_blueprint(stocks_bp)
register_legacy_stocks_routes(app)
...
app.teardown_appcontext(_db_close_db)
...
# --- STOCKS DOMAIN REDIRECT ---
# Nari Nakhre Stocks now lives on its own branded domain, www.stoqbell.com
# (same Render service, same app, same database -- just an additional
# custom domain attached to narinakhre-production in render.yaml). A
# /stocks* GET arriving on one of the storefront's own production hosts is
# permanently redirected to the same path on www.stoqbell.com so existing
# links/bookmarks keep working. Deliberately an explicit allow-list of old
# hosts (not "anything that isn't stoqbell") -- that keeps the test
# service, health checks, and the test client's default "localhost" host
# unaffected, so this doesn't send test/CI traffic off to production.
# Only GET is redirected: POST callbacks (Razorpay webhook, Kite postback)
# must be repointed at the provider's dashboard directly, since a 301
# silently turns a POST into a GET rather than reliably following it.
# Nothing outside /stocks is touched -- no other route. STOCKS_DOMAIN
# itself is defined in stoqbell/routes.py (imported above) -- Stocks owns
# its own domain name, the storefront just imports it for this redirect.
OLD_STOCKS_HOSTS = {'narinakhre.com', 'www.narinakhre.com', 'wholesale.narinakhre.com'}


@app.before_request
def redirect_stocks_to_own_domain():
    if not request.path.startswith('/stocks') or request.method != 'GET':
        return None
    host = request.host.lower().split(':')[0]
    if host not in OLD_STOCKS_HOSTS:
        return None
    target = f'https://{STOCKS_DOMAIN}{request.path}'
